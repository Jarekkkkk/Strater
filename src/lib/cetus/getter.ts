...
export const liquidityToCoinAmount = (
  ...
  const liquidity = new BN(liquidity_)
  const curSqrtPrice = new BN(current_sqrt_price)
  const lowerSqrtPrice = TickMath.tickIndexToSqrtPriceX64(lower_index)
  const upperSqrtPrice = TickMath.tickIndexToSqrtPriceX64(upper_index)
  ...
  return [
    amounts.coinA.toNumber() / 10 ** decimals_0,
    amounts.coinB.toNumber() / 10 ** decimals_1,
  ]
}
